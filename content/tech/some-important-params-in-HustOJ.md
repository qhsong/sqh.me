+++
Description = "HustOJ系统的几个关键参数说明"
Tags = ["HustOJ","OJ系统","参数"]
date = "2013-05-11T16:42:00+08:00"
title = "HustOJ评判系统的几个关键的参数的说明"
...
open_basedir =/home/judge/data:/var/www/JudgeOnline:/tmp:/var/www/phpMyAdmin
...
```
二、/home/judge/etc/judged.conf
这个文件是对judged进程的配置文件，以及解释。
```bash
...
```
基本没有太大的调整。主要是打开了SIM相似度检测功能，其实这个相似度检测比较弱，我试过，加一段无用的代码SIM就基本废了。。
三、/var/www/include/db_info.inc.php
```PHP
...
修改了语言，关闭了在线字典。
四、调试模式
Judged和Judge_client都是可以调试的，方法如下：
judged调试模式。
...
如：`
judge_client 2001 5 /home/judge/demo debug `将在/home/judge/demo/run5目录中对2001号提交进行重判，并打开调试模式，输出大量调试信息，运行后不删除中间结果。 这个模式可以帮助调试题目数据，发现数据问题和了解提交RE的详细错误原因。
